...
if(isset($postdata) && !empty($postdata))
{
  ...
  $likes = mysqli_real_escape_string($conn, trim($request->likes));

  print_r($request);

  // Update.
  $sql = "UPDATE `messages` SET `name`='$name',`heading`='$heading',`message`='$message',`likes`='$likes' WHERE `id` = '{$id}' LIMIT 1";
