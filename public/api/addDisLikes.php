...
if(isset($postdata) && !empty($postdata))
{
  ...
  $dislikes = mysqli_real_escape_string($conn, trim($request->dislikes));

  print_r($request);

  // Update.
  $sql = "UPDATE `messages` SET `name`='$name',`heading`='$heading',`message`='$message',`dislikes`='$dislikes' WHERE `id` = '{$id}' LIMIT 1";
